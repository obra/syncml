...
sub _syncable_for_ticket {
    ...
    my $ic = Data::ICal->new(vcal10 => 1);
    my $todo = Data::ICal::Entry::Todo->new;
    $ic->add_entry($todo);
    $todo->add_properties( summary => $ticket->Subject,
                           description => $self->_ticket_description($ticket) );

    for my $prop (qw/description/) {
        $todo->property($prop)->[0]->encode('QUOTED-PRINTABLE');
    }
...
sub _ticket_description {
    ...
    my $report = 
        qq{[This generated by RT (at @{[ $now->hms ]}); edits to it will be ignored.]\n\n};
    
    $report .= qq{From queue: @{[ $ticket->QueueObj->Name ]}; Status: @{[ $ticket->Status ]}};
...
sub add_item {
    ...
    if ($just_checking) {
        ...
        $ticket->Create( Queue => $q->id, 
            Subject => $ic->entries->[0]->property("summary")->[0]->decoded_value,
            Owner => $cu->Id);
